...
import useNFCCardSession from './hooks/useKeycardNFC';
...
const Main = () => {
  ...
  const [signResponseData, setSignResponseData] = React.useState<SignData | undefined>(undefined);
  ...
  const pathRef = React.useRef<string>(undefined);
  const messageRef = React.useRef<string>(undefined);
  const { start,
    stop,
    handleCardInitialized,
    handleCardAuthentic,
    handleCardPaired,
    handleSecureChannelOpened,
    handlePinVerified,
    handleCmdExecuted } =
  useNFCCardSession({
    log: log,
    setCardPhase: setPhase,
    modalActive: setIsModalVisible,
  ...
  const {
    ...
    setSignResponse: setSignResponseData,
  ...
  const checkCmdExec = React.useCallback((responseData: KeycardManagerResponse, successMessage?: string) => {
    ...
    if (responseData.status == 'error') {
      ...
      if ((screen != Screen.CreateMnemonic) && (screen != Screen.ShowWallet) && (screen != Screen.Sign)) {
        setScreen(Screen.Home);
      }

      if ((lastScreenRef.current != undefined) && ((screen == Screen.CreateMnemonic && lastScreenState != MnemonicSteps.ShowMnemonic) || screen == Screen.LoadMnemonic)) {
  ...
  const handleSignScreenClose = React.useCallback(() => {
    setEthAddresses([]);
    setSignResponseData(undefined);
    setScreen(Screen.Home)
  }, [ethAddresses, signResponseData]);
  ...
  const handleSignMessage = React.useCallback((p: string, message: string, path: string) => {
    pinRef.current = p;
  ...
  }, [stop, createMnemonic, loadMnemonic, exportKey, removeKey, changePIN, changePUK, getCardInfo, changePairing, unpair, unpairOthers, phase, log, screen, cardInfo]);
